...
@Component({
  selector: 'app-oshhcdashboard',
  templateUrl: './oshhcdashboard.component.html',
  styleUrls: ['./oshhcdashboard.component.css']
})
export class OshhcdashboardComponent implements OnInit {
  list:any;
  fullname:string;
  empimage:any;
  ...
  downloadexl(){
    this.firebaseService.downloadoshrchcsvalldata().subscribe(data=>{
     ...
     deleteevent(item){
        this.firebaseService.deleteoshrhcEvent(item.id,item.randomId);
   ...
   this.firebaseService.getselecteduserdataoshrhc(item.randomId).subscribe(data=>{
    ...
    generatepdf(item){
          this.firebaseService.searchmemberbyrandomidoshrhc(item.randomId).subscribe(edata=>{
            edata.map(em=>{
           ...
           doc.text("OSHRHC- Employee Data",50,40);
           doc.setFontSize(13);
           doc.setFontType("bold");
    ...
    edit(item){
     ...
     this.router.navigateByUrl('updateoshrhcmember');
    } 
  ngOnInit(): void {  
    ...
    this.firebaseService.getallemployeedatafromoshrhc().subscribe(data=>{
      this.users$ = data.map(e => {
        return {
          //id: e.payload.doc.id,
          id:e.payload.doc['id'],
