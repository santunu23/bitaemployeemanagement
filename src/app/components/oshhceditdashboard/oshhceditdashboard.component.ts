...
@Component({
  selector: 'app-oshhceditdashboard',
  templateUrl: './oshhceditdashboard.component.html',
  styleUrls: ['./oshhceditdashboard.component.css']
})
export class OshhceditdashboardComponent implements OnInit {
  randomID;
  lists;
  fullname;
  successshow;
...
constructor(
  ...
  ngOnInit(): void {
   ...
   this.randomID=this.cookieservice.get('membercheck');
    this.firebase.searchmemberbyrandomid(this.randomID).subscribe(data=>{
  ...
  async onSubmit(form: NgForm){
    ...
    if(emnid===undefined||emnid===false||emnid===null){
      ...
      console.log(res);
      this.firebase.updatememberdata(res).then(kam=>{
        this.spinner.hide();
        this.router.navigateByUrl('dashboard');
      })
  }
